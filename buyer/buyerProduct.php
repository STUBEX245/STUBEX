...
$result=$conn->query($sql);
  ...
  $sql="select * from product where shelf='on' and category ='$select_category' and quantity>0 and price BETWEEN '$min' AND '$max' and title like '%$keyword%' ORDER BY price limit $start_from,$num_per_page;;";
  $result=$conn->query($sql);
}
...
<ul class="menu">
  ...
  <li><a href="about_us.php">About</a></li>
    <li style="list-style-type: none;margin-top:4px;"><a href="chat.php"><i class="fa fa-comment" style="font-size:15px;color:white;"></i></a></li>
...
<form id="Formid" method="post" action="">
  <div class="search">
          <input type="text" name="search_product" id="search_product" value="<?php echo $keyword ?>" placeholder="Search for book title, semester and ....">
  </div>
          ...
          <div class="form-price-range-filter">
			...
			<p>Max Price</p>
			    ...
			    <input type="" id="min" name="min_price"
                    value="<?php echo $min; ?>" placeholder="Min Price">
                <div id="slider-range"></div>
                <input type="" id="max" name="max_price"
                    value="<?php echo $max; ?>" placeholder="Max Price">
            </div>
            <div>
...
if (empty($_POST['category'])) {
  ...
  $total_record = mysqli_num_rows($pr_result);
                ...
                if($page>1)
                {
                    echo "<a href='buyerProduct.php?page=".($page-1)."' class='btn btn-danger'>Previous</a>";
                }

                
                for($i=1;$i<$total_page;$i++)
                {
                    echo "<a href='buyerProduct.php?page=".$i."' class='btn btn-primary'>$i</a>";
                }

                if($i>$page)
                {
                    echo "<a href='buyerProduct.php?page=".($page+1)."' class='btn btn-danger'>Next</a>";
                }
